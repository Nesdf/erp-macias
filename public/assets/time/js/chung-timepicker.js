	...
	$.chungTimePicker = function(element, options) {
		...
		function createHoursWrap() {
			var table = $('<table class="table-condensed"></table>'),
				title = $('<thead><tr><th colspan="6" class="title">Hora</th></tr></thead>'),
				hours = $('<tbody></tbody>'),
		...
		function createBottomOperate() {
			switch(obj.options.viewType) {
				case 0:
					obj.timepicker_bottom.empty().append('')
						.append('<span class="bottom-btn js-cancel">Cancelar</span>');
					break;
				case 1:
					obj.timepicker_bottom.empty().append('')
						.append('<span class="bottom-btn js-cancel">Cancelar</span>')
						.append('<span class="bottom-btn js-confirm">Ok</span>');
